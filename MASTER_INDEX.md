...
| INDUSTRIAL-5 | 🟢 blockout | 🟢 propia | 🟢 propio (viento sordo 260Hz + zumbido de maquinaria) | 🟢 PBR real (hormigón/metal/metal oxidado) | 🔴 | 🔴 |
| OUTPOST-9 | 🟢 blockout (3 carriles) | 🟢 propia | 🟢 propio (viento silbante 750Hz) | 🟢 PBR real (hormigón/madera/tierra) | 🔴 | 🔴 |
...
**Resto de la directiva P0 (edificios/paredes/colisiones/balística)** — balística, colisión jugador-vs-pared, materiales/PBR, el primer interior real, y ahora también el controlador de movimiento con tolerancia de pendiente/escalón ya están resueltos y verificados. Queda como trabajo futuro explícito: interiores reales en más edificios (Control Tower/Warehouse/Office de KRYPTOS-URBAN, estructura de fábrica de INDUSTRIAL-5) y rampas/escaleras físicas conectando el walkway de INDUSTRIAL-5 y las torres de OUTPOST-9 al suelo (el controlador ya las soportaría, solo falta la geometría concreta en esos 2 mapas).

~~Controlador de movimiento: tolerancia real de pendiente/escalón~~ 🟢 completado. Causa raíz real identificada (no una limitación de `moveWithCollisions()`, que sí desliza correctamente por superficies inclinadas, sino el paso siguiente: un `groundLevel` de altura fija que cancelaba cualquier elevación ganada por esa colisión, forzando al jugador de vuelta abajo cada fotograma). Corregido con un nuevo registro `WALKABLE_SURFACES` (mismo patrón opt-in que `IMPACT_SURFACES`) y un rayo hacia abajo que sigue la altura real de cualquier superficie etiquetada, con una tolerancia de 0.6 unidades por fotograma (subida y bajada) — el suelo plano base de cada mapa no cambia en absoluto, cero riesgo de regresión ahí. Probado primero en aislamiento (ascenso continuo sin oscilar, caída correcta al salir de una plataforma sin barandilla) y luego en la rampa real de BACKLOT-7, reconstruida y ahora funcional (antes retirada por no funcionar). Regresión de suelo plano/pared/salto/puerta-del-interior/68-armas confirmada limpia. Alcance declarado: solo BACKLOT-7 recibió una rampa física esta pasada; INDUSTRIAL-5/OUTPOST-9 siguen sin la geometría de conexión aunque el controlador ya la soportaría. Ver `BULLET_OPS_PROGRESS.md` para el detalle completo.
...
**🎯 Próxima tarea real, sin esperar instrucción**: con el controlador de movimiento ya resuelto, el hueco de mayor prioridad que queda de la directiva P0 de edificios/paredes es dar rampas/escaleras físicas a INDUSTRIAL-5 (walkway) y OUTPOST-9 (torres) ahora que el controlador ya las soportaría sin trabajo adicional de movimiento — solo falta la geometría de conexión en cada mapa, siguiendo el mismo patrón ya verificado en BACKLOT-7. Alternativa de prioridad similar: más interiores reales (Control Tower/Warehouse/Office de KRYPTOS-URBAN, estructura de fábrica de INDUSTRIAL-5). Se abordará el de mayor apalancamiento como siguiente incremento verificado de principio a fin, salvo que llegue una nueva directiva del usuario con prioridad distinta.
