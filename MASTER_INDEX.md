...
**Siguiente concreto**: en cuanto el usuario adjunte la imagen de portada como archivo, integrarla en `#menuBg` (cambio de una línea ya preparado) y volver a probar visualmente. Mientras tanto, opciones razonables: FASE 13 fase 14 (muerte/ragdoll, complementa las animaciones recién añadidas) o fase 12 (sangre moderada, ya parcialmente cubierta por `spawnImpactEffect()`). Releer el mensaje truncado de ADS/miras cuando el usuario continúe esa idea, antes de tocar `WeaponController`'s ADS existente. La ampliación de roster de armas (FASE 4, AR 9/12→, SMG 7/10→) y el resto de items P4 quedan en pausa mientras esta nueva prioridad esté activa, pero no cancelados.
