...
**Siguiente concreto**: en cuanto el usuario adjunte la imagen de portada como archivo, integrarla en `#menuBg` (cambio de una línea ya preparado) y volver a probar visualmente. Mientras tanto, opciones razonables: retomar la ampliación de roster de armas (FASE 4, AR 9/12→, SMG 7/10→, en pausa desde que empezó esta redirección de sistema visual) o fase 7 (integrar animación de brazos con disparo/recarga real). Releer el mensaje truncado de ADS/miras cuando el usuario continúe esa idea, antes de tocar `WeaponController`'s ADS existente.
