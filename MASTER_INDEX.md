...
| INDUSTRIAL-5 | 🟢 blockout | 🟢 propia | 🟢 propio (viento sordo 260Hz + zumbido de maquinaria) | 🟢 PBR real (hormigón/metal/metal oxidado) | 🟡 rampa real al walkway elevado + interior real de factory1 + almacén NO (2/N edificios completos) | 🔴 |
...
| KRYPTOS-URBAN | 🟢 construido desde plano táctico + vídeo de referencia del usuario (3 rutas + 2 respawns + 3 balizas de Hardpoint posicionadas, lógica de modo pendiente) | 🟢 propia (luz rasante cálida) | 🟢 propio | 🟡 PBR real (concreto/ladrillo/metal/asfalto/madera/cristal, ver PROGRESS) — Office Building + Apartment Block (planta baja) + Destroyed Shop con interior real (3/N edificios completos) | 🟡 rampas reales a Control Tower/Warehouse (reemplazan escaleras que nunca funcionaron, ver PROGRESS) | 🔴 |
...
| **Texturas** | 🟢 mejora grande completada | Mapas de normales reales (`makeNormalNoise()`) añadidos a los 5 materiales opacos — antes ningún material tenía relieve, solo variación de color sobre geometría plana. Pasada de desgaste/manchas/arañazos añadida al color base de los 5 (churretes de lluvia en hormigón, humedad en juntas de ladrillo, arañazos en metal, manchas de aceite en asfalto, manchas de agua en madera). Mejora compartida: beneficia a los 4 mapas del proyecto, no solo KRYPTOS-URBAN |
| **Materiales** | 🟢 mejorado | 6 `makePBR*()` con metallic/roughness reales + ahora mapas de normales reales también. Pendiente: mapa de Ambient Occlusion dedicado y reflejos de entorno (sin HDR configurado, limitación real sin pipeline de assets) |
| **Iluminación** | 🟢 bloque de zonas oscuras cerrado | Verificado que los interiores sin ventana no son negro puro (luz hemisférica ambiental sí llega, comportamiento correcto). Bug real encontrado y corregido: el cielo/vacío (`scene.clearColor`) era un valor fijo casi negro compartido por los 4 mapas, contradiciendo el ambiente "diurno con neblina" de KRYPTOS-URBAN — ahora cada mapa tiene su propio tono de cielo coherente con su luz. Pendiente: revisión más fina de sombras/exposición por zona |
...
~~Segundo interior real: Office Building de KRYPTOS-URBAN~~ 🟢 completado. Mismo patrón de 4 paredes que BACKLOT-7 — oeste/este sólidas, sur con hueco de puerta de 3 unidades, norte convertida en ventana de cristal real de suelo a techo (antes solo cubría 3/8 de la altura). Probado con Playwright: entrada por la puerta confirmada, pared oeste sigue bloqueando desde fuera, disparo a través del cristal hace 36.96 de daño (exactamente la pérdida del 12% ya verificada en los otros 3 mapas). Ver `BULLET_OPS_PROGRESS.md`.

~~Tercer interior real: factory1 de INDUSTRIAL-5~~ 🟢 completado. Mismo patrón de 4 paredes — norte/sur/este sólidas, oeste con hueco de puerta de 4 unidades, techo nuevo cerrando la sala (interior oscuro sin ventanas, coherente con un edificio industrial). Un segundo hallazgo de metodología de pruebas (mismo patrón ya visto en la puerta de BACKLOT-7) llevó a una falsa alarma inicial de "el jugador atraviesa la pared sur" que resultó ser un artefacto del guion de prueba, no un bug real — confirmado con una traza fotograma a fotograma que la pared sí bloquea normalmente. Ver `BULLET_OPS_PROGRESS.md`.

~~Cuarto interior real: planta baja del Apartment Block de KRYPTOS-URBAN~~ 🟢 completado. Mismo patrón de 4 paredes, solo en la planta baja (hasta la primera losa "ledge" ya existente, que hace de techo) — los pisos superiores se quedan como bloque sólido, fuera de alcance declarado. Probado con Playwright (aproximación recta pre-alineada desde el principio, sin repetir el artefacto de metodología ya encontrado 2 veces): entrada por la puerta confirmada, pared oeste bloquea desde fuera, el techo (losa) impide subir a la planta superior saltando. Ver `BULLET_OPS_PROGRESS.md`. **Con esta pasada, los 4 mapas del proyecto tienen ya al menos un edificio con interior real transitable.**

~~Quinto interior real: Destroyed Shop de KRYPTOS-URBAN~~ 🟢 completado. Mismo patrón de 4 paredes — norte/este/oeste sólidas, sur con hueco de puerta de 3 unidades junto al prop de escombros ya existente. Probado con Playwright: entrada por la puerta confirmada, pared este bloquea desde fuera. Ver `BULLET_OPS_PROGRESS.md`.

~~Sexto interior real: almacén noroeste de INDUSTRIAL-5~~ 🟢 completado. Uno de los 4 almacenes de esquina (generados antes en bucle, idénticos) se saca del bucle y gana interior real — norte/sur/oeste sólidas, este con hueco de puerta de 4 unidades hacia el centro del mapa. Los otros 3 almacenes se quedan sólidos, alcance deliberado. Probado con Playwright: entrada confirmada, pared norte bloquea desde fuera. Ver `BULLET_OPS_PROGRESS.md`.

**🎯 Próxima tarea real, sin esperar instrucción**: seguir ampliando la cobertura de interiores reales (Destroyed Cafe/Maintenance Shed de KRYPTOS-URBAN, los otros 3 almacenes/factory2 de INDUSTRIAL-5, bases de OUTPOST-9) es la opción de mayor volumen, o la rampa en zigzag para la torre central/watchtower de OUTPOST-9 como alternativa de menor alcance. Se abordará como siguiente incremento verificado de principio a fin, salvo que llegue una nueva directiva del usuario con prioridad distinta.

## 🔫 FASE 15 — PRODUCCIÓN DEL BACKLOG DE LA WEAPON BIBLE (directiva del usuario, sesión actual)

Tras `docs/WEAPON_BIBLE.md`/`docs/weapons.json`/`docs/WEAPON_ASSET_BACKLOG.md` (auditoría honesta completa, ver esos archivos), el usuario dio luz verde para implementar el backlog P1 en el orden ya documentado: (1) Weapon Inspect, (2) identidad visual por arma, (3) VFX diferenciado por categoría, (4) `damageFalloff` extendido, (5) audio espacial.

~~Weapon Inspect: tecla configurable + movimiento de cámara + sonido mecánico~~ 🟢 completado. `gameSettings.inspectKey` (nuevo, default `i`, con el primer sistema mínimo de rebinding de teclas del proyecto — antes no existía ninguno), `WeaponController.getInspectCameraOffset()` (mueve `player.camera`, no solo el arma), `SoundSynth.playInspect()`. De paso, `!movementState.sprinting` añadido al trigger (comportamiento que la Weapon Bible marcó como "no definido"). Probado con Playwright end-to-end: rebind completo (incluido cancelar con Escape y abandonar Settings a medio rebind sin dejar el teclado bloqueado — bug real encontrado y corregido antes de cerrar el punto), la tecla vieja deja de funcionar y la nueva sí, bloqueo de disparo verificado con el timer de inspect en su valor máximo (no en el frame de finalización, donde un disparo sí es legítimo por diseño ya existente), sprint bloqueando el inicio de un inspect nuevo, sonido invocado exactamente 1 vez por activación. Ver `BULLET_OPS_PROGRESS.md` para el detalle completo, incluida la corrección de metodología de prueba (`#settingsBtn` está oculto a propósito, el punto de entrada real es `#navAjustesBtn`) y el ajuste por el framerate real de este sandbox (~4-5fps, no 60fps).

~~Identidad visual por arma: 11 armas insignia~~ 🟢 completado (parcial por diseño — 11 de 68, ver alcance honesto abajo). `buildWeaponViewmodel(scene, category, weaponId)` gana un tercer parámetro opcional; 11 ramas nuevas `if (weaponId === 'BOxx')` (una por categoría, la arma "plantilla" original de cada una: BO01/11/21/31/32/41/51/52/61/71/81) con piezas geométricas reales que ninguna otra arma de su categoría tiene (asa de transporte, culata plegable, guardamano de bombeo, bípode+reposa-mejilla, freno de boca, caja de munición lateral, compensador, martillo expuesto, placa de hombro, hoja curva tipo karambit con un nuevo helper `addTorus()`, silueta real de ballesta) — la cadena `if/else` por categoría que ya existía sigue intacta para las 57 armas restantes. Ambos puntos de construcción de viewmodel (partida real + previsualización de ARSENAL/CREAR CLASE/TIENDA) pasan ahora el `id` del arma. Probado con Playwright: las 11 construyen con las piezas extra esperadas (comparado por nombre de malla real), sus 11 pares no-insignia (ej. BO02 junto a BO01) producen exactamente la misma geometría con o sin pasar su propio id (la rama genérica queda intacta), capturas de pantalla de 4 de las 11 confirman visualmente que las piezas están en su sitio, y BO01 se equipó y disparó de verdad en una partida real sin errores. **Alcance honesto**: quedan 57 armas compartiendo silueta de categoría — extenderlo a las 68 es un incremento de producción mucho mayor, en el backlog aparte (`docs/WEAPON_ASSET_BACKLOG.md`, tickets M-*). Ver `BULLET_OPS_PROGRESS.md`.

~~VFX de disparo diferenciado por categoría~~ 🟢 completado. `MUZZLE_FLASH_PROFILES`, un perfil por cada una de las 10 categorías no-melee (tamaño de sprite/duración/nº de partículas/tamaño/vida/tasa de emisión/potencia/color1/color2 propios) en vez de la función `spawnMuzzleFlash()` única compartida por las 68 armas de antes — Pistol el más pequeño y rápido, Shotgun/Rocket/LMG los más grandes, Special con acento azulado en vez del naranja estándar de las demás. `getSharedMuzzleFlashMaterial()` pasa de un material único a un cache por color, mismo principio anti-fuga que ya se aplicó a otras partes del proyecto. Probado con Playwright: las 10 categorías producen mallas/sistemas de partículas realmente distintos (diámetro/color/capacidad comprobados leyendo el objeto real), fallback seguro ante una categoría desconocida, y una Shotgun equipada y disparada de verdad en partida real sin errores. Ver `BULLET_OPS_PROGRESS.md`.

~~Extender `damageFalloff` a las armas restantes~~ 🟢 completado (56/68, 12 excluidas a propósito). Fórmula derivada de las 3 armas ya existentes (`end` = `range` propio, `start` = fracción de `range` por categoría, `minMul` fijo por categoría), aplicada mediante un script de Node que localiza cada bloque de arma dentro de `WEAPON_CONFIGS` por conteo de llaves e inserta una sola línea nueva por arma, sin reescribir el resto del bloque. Excluidas a propósito: 6 Rocket Launcher + BO82 (splash, no bala que recorre distancia) + 5 Melee (golpe instantáneo). Probado: 68/68 armas cargan sin error de sintaxis, `damageFalloffMultiplier()` verificada directamente contra 5 armas recién extendidas con el comportamiento exacto esperado (plano hasta `start`, interpolación lineal, clamp en `minMul`), Rocket/Melee/BO82 confirmados sin regresión (siguen planos), y una bala real creada al disparar BO02 en partida lleva el `damageFalloff` correcto de su config. `docs/weapons.json` y `docs/WEAPON_BIBLE.md` resincronizados. Ver `BULLET_OPS_PROGRESS.md`.

~~Audio espacial básico con `PannerNode`~~ 🟢 completado (solo el disparo, por ahora). `SoundSynth.updateListener()` sigue a la cámara cada fotograma (`camera.globalPosition`/`getDirection()`, con fallback a la API antigua de Web Audio); `tone()`/`noiseBurst()` ganan un parámetro opcional `position` que las enruta por un `PannerNode` real (`distanceModel: 'inverse'`, `panningModel: 'equalpower'`) en vez de conectar directo a `masterGain` — el resto de sonidos del juego, que no pasan posición, siguen exactamente igual que antes. `playGunshot()` reutiliza la misma `muzzleWorldPos` que ya se calculaba para el muzzle flash. Probado con Playwright interceptando `AudioContext.prototype.createPanner` para inspeccionar el grafo de audio real: el listener sigue la cámara con ~0.03 unidades de desfase en juego normal, un sonido no posicional (`playEquip`) confirmado que no crea ningún panner, un disparo real crea exactamente 2 (uno por cada llamada interna de `playGunshot`), y disparar con el jugador muy lejos no lanza excepciones. Un primer intento de la prueba con el jugador teletransportado a media caída dio un desfase aparente de 1.0 unidad — investigado y confirmado que era un artefacto de esa teleportación anómala (la caída por gravedad no había terminado de resolverse en el instante exacto de la lectura), no un bug real, descartado repitiendo la prueba en juego normal. Ver `BULLET_OPS_PROGRESS.md`.

**Con este cambio se completan los 5 puntos del backlog P1 de la Weapon Bible** (`docs/WEAPON_BIBLE.md`, sección 14): identidad visual por arma, Weapon Inspect, VFX diferenciado, `damageFalloff` extendido, y audio espacial básico.

## 🔧 BACKLOG P2 DE LA WEAPON BIBLE

~~Primer accesorio real: slot OPTIC (Red Dot/Holographic/ACOG)~~ 🟢 completado (parcial por diseño — solo OPTIC, ver alcance honesto abajo). `ATTACHMENT_REGISTRY` estaba vacío y `getEffectiveWeaponStats()` (ya existente) nunca se llamaba desde `WeaponController` — ahora `currentConfig` pasa por ella de verdad, con caché invalidada al cambiar de arma o equipar en caliente. 3 accesorios reales con tradeoffs de diseño (Red Dot: ADS rápido sin zoom; Holo: intermedio; ACOG: zoom real + ADS más lento), `isSlotCompatible()` bloquea Melee (sin ADS real), representación visual real montada sobre la pieza sight/scope/optic existente o una posición por defecto, UI real en la pantalla de detalle de arma (fila "ÓPTICA" con swatches, patrón idéntico a camuflaje), persistencia por arma. Probado con Playwright: flujo de UI completo (equipar/desequipar vía clic real), incompatibilidad de Melee rechazada tanto en UI como en el dato, persistencia tras recargar, y el efecto real en partida verificado leyendo `wc.currentConfig` directamente (ADS time/fovMul cambian según el accesorio equipado, revierten inmediatamente al desequipar en caliente sin cambiar de arma). 0 errores. **Alcance honesto**: solo OPTIC poblado — GRIP/MAGAZINE/BARREL/STOCK quedan como tickets separados del mismo backlog, mismo patrón. Ver `BULLET_OPS_PROGRESS.md`.

~~Camera shake dedicado al disparo~~ 🟢 completado (ticket V-CAMSHAKE-FIRE). `WeaponController.camShakeAmount`/`camShakePhase` + `getCamShakeOffset()` (temblor multi-seno), acumulado por disparo vía `cfg.recoil.cameraKick × 2.2` (magnitud ya tuneada por arma — Shotgun sacude más que Pistol sin tabla nueva), decae rápido e independiente del `recoverySpeed` de cada arma, aplicado solo al `camera.rotation` final — nunca a `player.pitch`/`player.yaw`, así que nunca puede afectar a dónde va el siguiente disparo (eso sigue siendo del patrón de recoil real). Probado con Playwright: shake real tras disparar BO01 (0→0.0352, exacto), aim real (`pitch`/`yaw`) sin tocar, decae a 0 tras 20 fotogramas, y Shotgun (0.088) sacude casi el doble que Pistol (0.0484) en la proporción exacta de sus `cameraKick`. Ver `BULLET_OPS_PROGRESS.md`.

~~Trazadora de bala~~ 🟢 completado (ticket V-TRACER). `spawnBulletTracer()` — caja delgada emparentada a la bala (se mueve y se destruye sola con ella), orientada con la misma convención yaw/pitch ya usada en todo el archivo, largo escalado por `bulletSpeed`, material reutilizado del jugador/bot. Excluida de Melee/Explosivas a propósito. Cableada tanto en `WeaponController.fire()` como en `Bot.fire()`. Probado con Playwright: trazadora real con largo correcto, ausente en Melee/Rocket, limpieza sin mallas huérfanas confirmada. Ver `BULLET_OPS_PROGRESS.md`.

**🎯 Próxima tarea real, sin esperar instrucción**: seguir con el resto del backlog P2 (poblar GRIP/MAGAZINE/BARREL/STOCK siguiendo el mismo patrón que OPTIC, ampliar skins de 5 a 10 con rareza real, shell casing/smoke, animación de recarga visual) o retomar la tarea #84 (auditoría de KRYPTOS-URBAN), salvo que llegue una nueva directiva del usuario.
