...
| KRYPTOS-URBAN | 🟢 construido desde plano táctico + vídeo de referencia del usuario (3 rutas + 2 respawns + 3 balizas de Hardpoint posicionadas, lógica de modo pendiente) | 🟢 propia (luz rasante cálida) | 🟢 propio | 🟡 PBR real (concreto/ladrillo/metal/asfalto/madera/cristal, ver PROGRESS) pero sin interiores reales todavía | 🔴 | 🔴 |

| Tarea transversal de mapas | Estado | Prioridad |
|---|---|---|
| Texturas de superficie reales (ahora color plano en BACKLOT-7/INDUSTRIAL-5/OUTPOST-9, PBR real ya en KRYPTOS-URBAN) | 🟡 P0 — directiva activa del usuario: extender los 6 materiales PBR de KRYPTOS-URBAN a los otros 3 mapas + variación/desgaste/decals/grafitis, ver PRÓXIMA TAREA | |
| Interiores reales en edificios accesibles | 🔴 P0 — directiva activa del usuario, ver PRÓXIMA TAREA | |
| Colisiones: auditoría completa pared/edificio/puerta/vehículo | 🔴 P0 — directiva activa del usuario, ver PRÓXIMA TAREA | |
| Balística: las balas NO deben atravesar paredes sólidas | 🔴 P0 — bug real confirmado en el código actual (ver PRÓXIMA TAREA): el hit-test de personajes se ejecuta ANTES que el raycast de entorno, así que un disparo puede dañar a un jugador protegido detrás de una pared tageada como sólida | |
| Penetración de balas por material (cristal fácil, madera con pérdida de daño, hormigón/metal grueso detienen la bala) | 🔴 P0 — directiva activa del usuario, ver PRÓXIMA TAREA | |
...
**P0 — Nueva directiva activa del usuario (edificios/paredes/colisiones/balística), reemplaza la prioridad de roster de armas mientras esté abierta**: el usuario pidió una revisión completa de materiales de edificios (PBR real por superficie, sin textura única repetida), interiores reales en edificios accesibles, auditoría de colisiones (paredes/puertas/vehículos), y sobre todo **corregir la balística para que las paredes bloqueen de verdad los disparos** — con penetración realista por material (cristal fácil, madera con pérdida de daño, hormigón/metal grueso detiene la bala). **Bug real ya confirmado leyendo el código** (antes de tocar nada): en el bucle de balas, el hit-test contra personajes (`for (let entity of [player, ...bots])`) se ejecuta y decide el impacto ANTES que el raycast de entorno (`IMPACT_SURFACES`, que solo corre `if (!hit && ...)`) — así que hoy, un disparo cuya línea recta pasa cerca de un jugador detrás de una pared tageada como sólida SÍ le hace daño, porque nunca se comprueba si esa pared bloquea la línea de visión antes de acreditar el impacto. Esto es exactamente el bug "ARMA → PARED → JUGADOR = debería ser SIN DAÑO" que describió el usuario. Plan concreto: (1) fusionar ambos raycasts en una sola comprobación por bala que determine cuál de los dos (personaje o superficie de entorno) está más cerca a lo largo del segmento de ese fotograma — la superficie de entorno gana si está más cerca; (2) para superficies con perfil de penetración (`glass`/`wood`/futuro `door`), permitir que la bala continúe con daño reducido y una superficie más atravesada, en vez de terminar ahí; (3) extender los 6 materiales PBR de KRYPTOS-URBAN a BACKLOT-7/INDUSTRIAL-5/OUTPOST-9; (4) interiores reales, priorizando los edificios ya "jugables" en cada mapa (Control Tower/Warehouse/Office en KRYPTOS-URBAN, estructuras de fábrica en INDUSTRIAL-5); (5) Playwright real disparando contra pared/esquina/puerta/ventana/vehículo desde varias distancias/ángulos, y un caso explícito de "jugador protegido detrás de pared no recibe daño" — corrigiendo cualquier bug encontrado automáticamente, no solo reportándolo.

**Pendiente aparte (menor prioridad, en pausa mientras la directiva de arriba esté activa)**: en cuanto el usuario adjunte la imagen de portada del menú como archivo, integrarla en `#menuBg`. Terminar de probar/documentar/commitear la ampliación de roster AR/SMG (BO94 HAILSTORM/BO95 JUDGMENT/BO18 THREEPOINT/BO19 DELUGE) que quedó escrita pero sin probar cuando llegó esta redirección. Releer el mensaje truncado de ADS/miras cuando el usuario lo continúe, antes de tocar el ADS existente de `WeaponController`.
