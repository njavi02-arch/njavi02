...
| 8 — HUD | 🔴 pendiente | HUD actual (vida/munición/arma/cargador) ya existe de fases anteriores — faltan minimapa y elementos nuevos pedidos |
| 9 — Crosshair | 🟡 parcial | Ya existe una retícula reactiva por categoría de arma (FASE 5) — falta el crosshair clásico de 4 líneas específico que pidió el usuario como base, con la reactividad por categoría como variación |
| 10 — Minimapa | 🔴 pendiente | No existe. Pedido: esquina inferior izquierda, tiempo real, jugador/equipo/orientación, configurable después |
| 11 — Impactos | 🟡 parcial | Ya existe `spawnImpactEffect()` genérico — falta diferenciar por material (metal/madera/hormigón/tierra/vidrio) |
...
| 13 — Hitmarker | 🟡 parcial | Ya existe `showHitmarker()` + sonido diferenciado en headshot (FASE 4/5) — falta variante visual distinta para headshot y opciones de configuración (tamaño/duración/color) |
...
**Siguiente concreto**: FASE 13, fase 5 (sistema de skins/variantes) o fase 8-10 (HUD/crosshair clásico/minimapa) — cualquiera de las dos es un siguiente paso razonable; se priorizará minimapa+crosshair clásico primero por ser lo más visible en cada partida, luego variantes de skin. Releer el mensaje truncado de ADS/miras cuando el usuario continúe esa idea, antes de tocar `WeaponController`'s ADS existente. La ampliación de roster de armas (FASE 4, AR 9/12→, SMG 7/10→) y el resto de items P4 quedan en pausa mientras esta nueva prioridad esté activa, pero no cancelados.
