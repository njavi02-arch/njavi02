...
- Caída de daño por distancia (damage falloff curve) — actualmente `damage` es un valor fijo sin min/max por rango. **Próxima tarea concreta**, ver PRÓXIMA TAREA.
...
**Siguiente concreto (P1, gap real identificado en la directiva del usuario)**: implementar **caída de daño por distancia (damage falloff)** como tercera mecánica real — actualmente `damage`/`pelletDamage` son valores fijos sin variación por rango, así que un impacto a 1 unidad y uno al borde del `range` configurado hacen el mismo daño. Requiere: nuevos campos por arma (p. ej. `damageFalloff: { start, end, minMul }` — daño completo hasta `start`, caída lineal hasta `minMul` en `end`) y aplicar el multiplicador en el punto de impacto de `updateGame()` usando la distancia real recorrida por la bala (`bullet.distanceTraveled`), igual que ya se usa para expirar por rango. Verificar con Playwright que un impacto cercano hace el daño completo y uno cerca del límite de alcance hace menos. Después de eso: retomar la ampliación de roster (FASE 4, P2) por AR/SMG (9/12, 7/10 — SMG con más hueco absoluto pendiente). Después: revisar el 2º pase de props de mapas (FASE 6) por si quedan zonas vacías tras la última pasada. El resto de items P4 (4º mapa, música de menú/partida, CUSTOMIZE bloqueado en skins) y Fase 10 multijugador siguen en pausa.
