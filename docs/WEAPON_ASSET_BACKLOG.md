...
| V-SHELL | Eyección de casquillos | 🟢 COMPLETADO | `spawnShellCasing()`/`updateShellCasings()`, cilindro con velocidad+gravedad+giro a mano, 1 por disparo (no por perdigón), excluido de explosivas. Ver `BULLET_OPS_PROGRESS.md` |
...
| # | ASSET | ESTADO | NECESIDAD | ESPECIFICACIÓN |
|---|---|---|---|---|
| K-COUNT | Ampliar de 5 a 10 skins mínimas | 🟢 COMPLETADO | Diseñar 5 skins nuevas siguiendo el mismo patrón que `digital` (patrón real dibujado en `DynamicTexture`, no solo color plano) | 5 nuevas: Overgrowth (blotches orgánicos), Riptide (bandas de onda), Rustbelt (speckle de óxido), Glitch Array (scanlines RGB-shift), Prism Shift (gradiente iridiscente) — nomenclatura por familias temáticas (nature/digital) inspirada en la organización de `github.com/tomaszobac/bf6-camo-list` (solo convención de nombres, ningún asset/pixel copiado). Ver `BULLET_OPS_PROGRESS.md` |
| K-RARITY | Sistema de rareza real (5 tiers) | 🟢 COMPLETADO | Extender el campo a COMMON/UNCOMMON/RARE/EPIC/LEGENDARY, remapeando las 5+5=10 skins finales | `SKIN_TIER_COLOR` nuevo (color real por tier en el badge de TIENDA, antes fijo en verde), precios escalados por tier en `SKIN_PRICES`. Ver `BULLET_OPS_PROGRESS.md` |
